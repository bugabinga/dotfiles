...
class aes {
  ...
  static byte[] SALT = new byte[] { 116, 104, 101, 32, 109, 111, 111, 110, 32, 105, 115, 32, 104, 105, 100, 100, 101,
      110, 32, 105, 110, 32, 112, 108, 97, 105, 110, 32, 118, 105, 101, 119 };
  ...
  /*
   * AES encrypts 128-blocks internally, so this should be reasonable memory bufer
   * size.
   */
  static int READ_BUFFER_SIZE = 128;
  /*
   * Supported sizes for GCM authentication tags seem to be {128, 120, 112, 104,
   * 96}.
  ...
  public static void main(String[] arguments) throws Throwable {
    ...
    Command command;
    if ("encrypt".startsWith(input_command)) {
      command = Command.ENCRYPT;
    } else if ("decrypt".startsWith(input_command)) {
      command = Command.DECRYPT;
    } else {
      throw fail("unknown command %s given%n", emphasize_global(input_command));
    }
     ...
     * In such cases a `--password-file` flag is mandatory, because we cannot
    ...
    if (console == null) {
      if (arguments.length != 3 || !"--password-file".equals(arguments[1])) {
        usage();
        throw fail(
            "When running %s non-interactively, the %s flag is required.%nOtherwise there is no way to securely obtain the password.%n",
            emphasize_local("aes"), emphasize_global("--password-file"));
      }
      var password_file = Path.of(arguments[2]);
      if (!Files.exists(password_file)) {
        throw fail("the given password file %s does not exist%n", emphasize_local(password_file.toString()));
      }
      ...
      if (!Files.exists(input_path)) {
        throw fail("the given input path %s does not exist%n", emphasize_global(input_path.toString()));
      }
      var output_path = Path.of(output);
      if (Files.exists(output_path)) {
        throw fail("the output path %s already exists%n", emphasize_global(output_path.toString()));
      }

      try (var input_stream = Files.newInputStream(input_path, READ);
          var output_stream = Files.newOutputStream(output_path, CREATE_NEW, WRITE);) {
        switch (command) {
          case ENCRYPT:
  ...
  static void encrypt(char[] password, InputStream input, OutputStream output) throws Exception {
    ...
    // save the IV for the decryption step later. secure? nope. but how else are we
    // to share IVs between encryption/decryption?
  ...
  static void decrypt(char[] password, InputStream input, OutputStream output) throws Exception {
    ...
    // the java impl of GCM seems to buffer the whole data in memory and only
    // releasing it on doFinal. yuck!
    ...
    output.write(AES_GCM_PKCS5.doFinal());
    // FIXME: when given a wrong password we get a nonsensical error. make this
    // nicer for human
  }
  ...
  static void usage() {
    ...
    out.printf("%s: java aes.java <e(ncrypt)|d(ecrypt)> [--password-file <path>] <input path> <output path>%n",
        emphasize_global("Usage for files"));
    out.printf("%s: java aes.java <e(ncrypt)|d(ecrypt)> --password-file <path>%n",
        emphasize_global("Usage for stdin/stdout"));
  }

-- taken from https://gist.github.com/runiq/31aa5c4bf00f8e0843cd267880117201
...
function _.throttle_leading( fn, ms )
  td_validate( fn, ms )
...
--@returns (function, timer) Throttled function and timer. Remember to call
---`timer:close()` at the end or you will leak memory!
function _.throttle_trailing( fn, ms, last )
  td_validate( fn, ms )
  local timer = vim.loop.new_timer()
  local running = false
...
---`timer:close()` at the end or you will leak memory!
function _.debounce_leading( fn, ms )
  td_validate( fn, ms )
  local timer = vim.loop.new_timer()
  local running = false
...
--@returns (function, timer) Debounced function and timer. Remember to call
---`timer:close()` at the end or you will leak memory!
function _.debounce_trailing( fn, ms, first )
  td_validate( fn, ms )
  local timer = vim.loop.new_timer()
  local wrapped_fn
...
---strategy.
function _.test_defer( bouncer, ms, firstlast )
  local bouncers = {
    tl = _.throttle_leading,
    tt = _.throttle_trailing,
    dl = _.debounce_leading,
    dt = _.debounce_trailing,
  }

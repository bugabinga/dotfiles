...
return  {
	...
	config = function(_, opts)
		...
		map.normal.terminal.insert {
			name = 'Toggle markdown preview via mdcat',
			category = 'terminal',
			keys = '<F9>',
			command = Mdcat,
		}
  ...
  -- [ GRAPHVIZ ]
		...
		_G.Graphviz = function()
			local window = vim.api.nvim_get_current_win()
			local current_file = vim.api.nvim_buf_get_name(0)
			graphviz.cmd = 'watch ' .. current_file .. ' { clear; dot -O -Tpng ' .. current_file .. ' ; viu --blocks ' .. current_file .. '.png }'
			graphviz:toggle()
			vim.api.nvim_set_current_win(window)
		end

		map.normal.terminal.insert {
			name = 'Toggle graphviz preview via viu',
			category = 'terminal',
			keys = '<F10>',
			command = Graphviz,
		}

...
local download_plugin_into = function ( puck_root, plugin_name, plugin_tarball_url, plugin_install_path )
  ...
  local tar_handle, tar_pid = vim.uv.spawn( 'tar',
                                            ...
                                            function ( code, signal )
                                              ...
                                              local contains_name = function ( name, path )
                                                return name:find( plugin_name,
                                                                  0, true )
                                              end
                                              local extracted_dir = vim.fs.find( contains_name,
                                                                                 { path = puck_root, type = 'directory', limit = 1, } )
                                              assert( #extracted_dir ~= 0, 'No directory was found in tarball' )
                                              assert( #extracted_dir == 1,
                                                      'Too many directories were found in tarball. Expected one!' )
                                              ...
                                              assert( ok,
                                                      string.format( 'Failed to move extracted plugin from %s to %s.',
                                                                     extracted_dir[1], plugin_install_path ) )
...
local install_plugin = function ( puck_root, plugin_type, plugin_coords, plugin_rev )
  ...
  if (vim.uv.fs_access( plugin_install_rev_path, 'R' )) then
    ...
    if not plugin_install_rev_file then
      vim.notify( 'Unable to read rev file ' .. plugin_install_rev_path,
  ...
  if not plugin_install_rev_file then
    vim.notify( 'Unable to write rev file ' .. plugin_install_rev_path,
                vim.log.levels.ERROR )
  end

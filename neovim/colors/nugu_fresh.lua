--- bugabingas color theme `nugu` for neovim

-- Reset all highlights if another colorscheme was previously set
if vim.g.colors_name ~= nil then vim.cmd [[ highlight clear ]] end

-- follow convention for others to be able to query current colorscheme
vim.g.colors_name = 'nugu_fresh'

local palette = {
  debug = { light = '#3dd45d', dark = '#3dd45d', },
  error = { light = '#740523', dark = '#f27a9a', },
  info = { light = '#4187a7', dark = '#e7f0f3', },
  warning = { light = '#a7802d', dark = '#f1e8d6', },

  content_normal = { light = '#494949', dark = '#cfcfcf', },
  content_backdrop = { light = '#e3e3e3', dark = '#131313', },
  content_accent = { light = '#800080', dark = '#800080', },
  content_minor = { light = '#878787', dark = '#727272', },
  content_focus = { light = '#646464', dark = '#a5bdb6', },
  content_unfocus = { light = '#c9c9c9', dark = '#3f4543', },
  content_important_global = { light = '#48256e', dark = '#8e58cc', },
  content_important_local = { light = '#643398', dark = '#7a3bc3', },

  ui_normal = { light = '#929292', dark = '#e2e2e2', },
  ui_backdrop = { light = '#f9f9f9', dark = '#000000', },
  ui_accent = { light = '#d43dd4', dark = '#7a067a', },
  ui_minor = { light = '#d0d0d0', dark = '#424242', },
  ui_focus = { light = '#646464', dark = '#788984', },
  ui_unfocus = { light = '#c9c9c9', dark = '#121212', },
  ui_important_global = { light = '#9170b5', dark = '#5f368d', },
  ui_important_local = { light = '#ae95c8', dark = '#4c2b72', },
}

local function choose( variants )
...
local debug = choose( palette.debug )
local error = choose( palette.error )
local info = choose( palette.info )
local warning = choose( palette.warning )

local content_normal = choose( palette.content_normal )
local content_backdrop = choose( palette.content_backdrop )
local content_accent = choose( palette.content_accent )
local content_minor = choose( palette.content_minor )
local content_focus = choose( palette.content_focus )
local content_unfocus = choose( palette.content_unfocus )
local content_important_global = choose( palette.content_important_global )
local content_important_local = choose( palette.content_important_local )

local ui_normal = choose( palette.ui_normal )
local ui_backdrop = choose( palette.ui_backdrop )
local ui_accent = choose( palette.ui_accent )
local ui_minor = choose( palette.ui_minor )
local ui_focus = choose( palette.ui_focus )
local ui_unfocus = choose( palette.ui_unfocus )
local ui_important_global = choose( palette.ui_important_global )
local ui_important_local = choose( palette.ui_important_local )
...
for name, group in pairs( _.groups ) do
  local namespace = 0
  -- vim.print( namespace, name, group )
  vim.api.nvim_set_hl( namespace, name, group )
end

-- vim.print( 'ALL HIGHLIGHTS', _ )
